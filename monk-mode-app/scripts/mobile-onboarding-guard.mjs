...
function hasFinalMarkers(source) {
  return source.includes("setStep((current) => Math.min(3, current + 1))")
    && source.includes("setStep((current) => Math.max(1, current - 1))")
    && source.includes("window.scrollTo({ top: 0, behavior: \"smooth\" })");
}
